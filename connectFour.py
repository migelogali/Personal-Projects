...
def gameplay_test():
    game = Connectfour()
    game._current_player = 1  # Start with Player 1
    print(f"Starting player: Player {game._current_player}\n")

    # Define moves as (player, column) sequence
    moves = [
        (1, 0),  # P1
        (2, 0),  # P2
        (1, 1),  # P1
        ...
        (1, 2),  # P1
        (2, 2),  # P2
        (1, 3),  # P1 -> this should be a horizontal win
    ]
    ...
    for player, col in moves:
        ...
        move = game._drop_checker(col)
        if move is None:
            print(f"Column {col} full! Move skipped.")
            continue
        row, col = move

        # Print the board after each move
        print(f"Player {player} moves at column {col}")
        game._print_board()
